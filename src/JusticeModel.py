...
class JusticeModel:
    ...
    def __init__(self, justice, mode='crossval', model=None):
        '''
        Initializes a JusticeModel object. Automatically loads the dataset with the associated justice
        
        Parameters:
        justice: string
        mode = ['crossval', 'full']
        model = [None, estimator object]
    ...
    def add_model(self, model):
        '''
        Takes an estimator object and adds it to the model attribute
        '''
        self.model = model
    ...
    def predict(self, X=np.array([])):
        '''
        If given a set of data, predicts and returns the values. If not, predict will predict using the stored X_test data.
        '''
        if len(X) == 0:
    ...
    def balance_train_data(self, X, y, method=None):
        '''
        Applies a balancing method to the data being passed in
        '''
        if method == None:
            return X, y
